...
export default function SupplierInventory() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <p className="text-sm text-[#475569] mb-6">재고 현황을 확인하고 출고를 관리합니다</p>
      ...
      <div className="bg-white border border-[#e2e8f0] rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-[#e2e8f0]">
            <div className="hidden md:grid grid-cols-[1fr_100px_80px_100px] gap-4 px-5 py-3 bg-[#f8f9fa] text-xs text-[#475569] uppercase tracking-wider">
              <span>부품명</span><span>재고</span><span>상태</span><span>액션</span>
            </div>
            {filtered.map(item => (
              <div key={item.inventoryId} className="grid grid-cols-1 md:grid-cols-[1fr_100px_80px_100px] gap-2 md:gap-4 px-5 py-3 hover:bg-[#f8f9fa] items-center">
                ...
                <div>
                  {item.stock > 0 && (
                    <button onClick={() => { setConsumeTarget(item); setConsumeQty(''); }} className="text-xs text-[#1d4ed8] hover:text-[#1d4ed8]">차감</button>
                  )}
                </div>
              </div>
            ))}
