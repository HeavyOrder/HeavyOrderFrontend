...
export default function NewOrder() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-xl font-bold text-[#f5f5f5] mb-1">새 발주</h1>
      <p className="text-sm text-[#666] mb-6">부품을 검색하고 발주합니다</p>
        ...
        {[1, 2, 3].map(s => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium ${
              step >= s ? 'bg-[#3b82f6] text-white' : 'bg-[#1a1a1a] text-[#666] border border-[#2a2a2a]'
            }`}>{s}</div>
            {s < 3 && <div className={`w-8 h-px ${step > s ? 'bg-[#3b82f6]' : 'bg-[#2a2a2a]'}`} />}
          </div>
        ))}
      ...
      {step === 1 && (
        <div>
          ...
          <div className="flex gap-2 mb-6">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 rounded-lg text-sm bg-[#111] border border-[#2a2a2a] text-[#e5e5e5] placeholder:text-[#666] focus:outline-none focus:ring-2 focus:ring-[#3b82f6]/30 focus:border-[#3b82f6]/30"
              />
            </div>
          ...
          {searchResults !== null && (
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-[#a0a0a0]">
                &quot;{keyword}&quot; 검색 결과 <span className="text-[#f5f5f5] font-medium">{searchResults.length}건</span>
              </p>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
              {displayParts.map(p => {
                ...
                return (
                  <div
                    key={p.id}
                    className={`bg-[#111] border rounded-xl p-4 transition-all duration-150 hover:border-[#333] ${
                      inCart ? 'border-[#3b82f6]/40' : 'border-[#2a2a2a]'
                    }`}
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm font-medium text-[#f5f5f5] truncate">{p.name}</h3>
                        <p className="text-xs text-[#666] mt-0.5">{p.supplier}</p>
                      </div>
                      {inCart && (
                        <span className="flex-shrink-0 ml-2 px-1.5 py-0.5 text-[10px] font-medium bg-[#3b82f6]/10 text-[#3b82f6] border border-[#3b82f6]/20 rounded">
                          {inCart.quantity}개
                        </span>
                      )}
                    </div>
                    <div className="flex items-end justify-between">
                      ...
                      <button
                        onClick={() => addToCart(p)}
                        className="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors duration-150 bg-[#1a1a1a] text-[#a0a0a0] border border-[#2a2a2a] hover:bg-[#222] hover:text-[#e5e5e5] hover:border-[#333]"
                      >
                        + 담기
                      </button>
                    </div>
          ...
          {/* 장바구니 미리보기 (하단 고정 스타일) */}
          {cart.length > 0 && (
            <div className="sticky bottom-4 bg-[#111] border border-[#f59e0b]/30 rounded-xl p-4 shadow-lg shadow-black/30">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-[#f59e0b]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                  </svg>
                  <span className="text-sm font-medium text-[#f5f5f5]">장바구니 ({cart.length}개 품목)</span>
                </div>
                <span className="text-sm font-bold font-mono text-[#f59e0b]">{formatCurrency(totalAmount)}</span>
              </div>
              <div className="space-y-1.5 mb-3 max-h-24 overflow-y-auto">
                {cart.map(c => (
                  <div key={c.product.id} className="flex items-center justify-between text-xs">
                    <span className="text-[#a0a0a0] truncate mr-2">{c.product.name} x{c.quantity}</span>
                    <span className="text-[#e5e5e5] font-mono flex-shrink-0">{formatCurrency(c.product.price * c.quantity)}</span>
                  </div>
                ))}
              </div>
      ...
      {step === 2 && (
        <div>
          <div className="bg-[#111] border border-[#2a2a2a] rounded-xl overflow-hidden mb-4">
            {cart.map(c => (
              <div key={c.product.id} className="flex items-center justify-between px-5 py-3 border-b border-[#2a2a2a] last:border-0">
                <div>
                  <p className="text-sm text-[#e5e5e5]">{c.product.name}</p>
                  <p className="text-xs text-[#666]">{c.product.supplier} · {formatCurrency(c.product.price)}/개</p>
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex items-center border border-[#2a2a2a] rounded">
                    <button onClick={() => updateQty(c.product.id, c.quantity - 1)} className="px-2 py-1 text-sm text-[#a0a0a0] hover:text-[#e5e5e5]">-</button>
                    <span className="px-2 py-1 text-sm text-[#e5e5e5] font-mono border-x border-[#2a2a2a]">{c.quantity}</span>
                    <button onClick={() => updateQty(c.product.id, c.quantity + 1)} className="px-2 py-1 text-sm text-[#a0a0a0] hover:text-[#e5e5e5]">+</button>
                  </div>
                  <span className="text-sm font-mono text-[#e5e5e5] w-24 text-right">{formatCurrency(c.product.price * c.quantity)}</span>
                  <button onClick={() => removeItem(c.product.id)} className="text-xs text-[#ef4444] hover:text-[#f87171]">삭제</button>
                </div>
              </div>
            ))}
          </div>
          <div className="flex items-center justify-between px-1 mb-4">
            <span className="text-sm text-[#a0a0a0]">합계</span>
            <span className="text-lg font-bold font-mono text-[#f5f5f5]">{formatCurrency(totalAmount)}</span>
          </div>
      ...
      {step === 3 && (
        <div>
          <div className="bg-[#111] border border-[#2a2a2a] rounded-xl p-5 space-y-4 mb-4">
          ...
          <div className="bg-[#111] border border-[#2a2a2a] rounded-xl p-5 mb-4">
            <h3 className="text-sm font-semibold text-[#f5f5f5] mb-3">주문 요약</h3>
            <div className="space-y-2 mb-3">
              {cart.map(c => (
                <div key={c.product.id} className="flex justify-between text-sm">
                  <span className="text-[#a0a0a0]">{c.product.name} x{c.quantity}</span>
                  <span className="text-[#e5e5e5] font-mono">{formatCurrency(c.product.price * c.quantity)}</span>
                </div>
              ))}
            </div>
            <div className="border-t border-[#2a2a2a] pt-3 flex justify-between">
              <span className="text-sm font-medium text-[#f5f5f5]">합계</span>
              <span className="text-base font-bold font-mono text-[#f59e0b]">{formatCurrency(totalAmount)}</span>
            </div>
          </div>

          {error && <div className="bg-[#ef4444]/10 text-[#ef4444] border border-[#ef4444]/20 p-3 rounded-lg text-sm mb-4">{error}</div>}
