...
export default function RepairShopSchedule() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-xl font-bold text-[#f5f5f5] mb-1">일정 관리</h1>
      <p className="text-sm text-[#666] mb-6">기사 예약 일정을 확인하고 관리합니다</p>
            ...
            {filtered.map(r => (
              <div key={r.id} className="bg-[#111] border border-[#2a2a2a] rounded-xl p-4 hover:border-[#333] transition-colors">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="text-sm font-medium text-[#f5f5f5]">{r.driverEmail}</h3>
                    <p className="text-xs text-[#666] mt-0.5 font-mono">{r.driverPhoneNumber}</p>
                  </div>
                ...
                {r.memo && (
                  <p className="text-xs text-[#666] bg-[#0a0a0a] rounded p-2 mb-3">{r.memo}</p>
                )}
